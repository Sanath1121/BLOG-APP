    ...
    <nav className={navbarClass}>
      <div className={navContainerClass}>
        ...
        <ul className={navLinksClass}>
          {isAuthenticated && (
            <li className="hidden md:block text-[0.72rem] text-[#6e6e73] bg-[#f5f5f7] border border-[#e8e8ed] rounded-full px-3 py-1">
              Session: <span className="text-[#1d1d1f] font-medium">{getDisplayName()}</span> ({user?.role ?? "NA"})
            </li>
          )}
